...
| `utils/RsqlOperationsRegistry.kt` | Backed by `ConcurrentHashMap`; public surface exposes defensive snapshot copies via `toMap()` / `toSet()` of `Map<ComparisonOperator, ProcessorParamsBuilder>` and `Set<ComparisonOperator>`; `registerOperation(operator, processor)` and `initDefaultPostgresOperation()` kept; COMMON entries seeded with factories that `TODO("Phase 2")` until processors land |
